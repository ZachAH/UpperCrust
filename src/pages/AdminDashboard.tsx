...
export default function AdminDashboard() {
  ...
  // Hours State (Matching your specific Firestore keys)
  ...
  const handleUpdateGlobalContent = async (e: FormEvent) => {
    ...
    if (view === "website") { docName = "homepage"; data = siteContent; }
    else if (view === "menu-page") { docName = "menu-page"; data = menuPageContent; }
    else if (view === "hours") { docName = "hours"; data = hoursContent; }
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-white font-sans overflow-hidden relative">
      ...
      {/* SIDEBAR */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-zinc-900 border-r border-zinc-800 p-6 flex flex-col transition-transform duration-300 md:translate-x-0 md:static ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 space-y-8 overflow-y-auto no-scrollbar">
          <div>
            ...
            {categories.map((cat) => (
              <button key={cat} onClick={() => { setView("menu"); setActiveCategory(cat); setIsSidebarOpen(false); }}
                className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold uppercase mb-1 transition-all ${view === "menu" && activeCategory === cat ? "bg-red-600 text-white" : "text-zinc-500"}`}>
                {cat.replace("_", " & ")}
              </button>
            ))}
          </div>
          <div>
            ...
            <button onClick={() => setView("website")} className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold uppercase mb-1 ${view === "website" ? "bg-blue-600" : "text-zinc-500"}`}>Home Page</button>
            <button onClick={() => setView("menu-page")} className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold uppercase mb-1 ${view === "menu-page" ? "bg-yellow-600" : "text-zinc-500"}`}>Menu Header</button>
            <button onClick={() => setView("hours")} className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold uppercase ${view === "hours" ? "bg-orange-600" : "text-zinc-500"}`}>Store Hours</button>
          </div>
        </nav>
        <button onClick={() => signOut(auth)} className="mt-6 p-4 text-[10px] text-zinc-600 border border-zinc-800 rounded-2xl font-black uppercase">LOGOUT</button>
      </aside>

      {/* MAIN CONTENT AREA */}
      <main className="flex-1 p-6 md:p-16 overflow-y-auto bg-black pt-24 md:pt-16">
        {loading ? (
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {view === "menu" && (
              <section>
                <div className="flex justify-between items-end mb-12">
                  ...
                  <button onClick={() => { setIsAddingNew(true); setEditingItem({ name: "", price: "", desc: "", subcategory: "Hoagies", available: true }); setIsModalOpen(true); }} className="bg-white text-black px-8 py-3.5 rounded-2xl font-black uppercase text-xs">+ Add Item</button>
                </div>
                  ...
                  <div key={subcat} className="mb-12">
                    ...
                    <div className="grid gap-3">
                      {getGroupedItems()[subcat].map((item: any) => (
                        <div key={item.id} className={`bg-zinc-900/40 border p-5 rounded-[24px] flex justify-between items-center transition-all ${item.available ? "border-zinc-800" : "border-red-900/50 opacity-60"}`}>
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              className={`px-4 py-2 rounded-full text-[9px] font-black uppercase border transition-all ${item.available ? "border-green-500/50 text-green-500 hover:bg-green-500/10" : "border-red-500 text-red-500 bg-red-500/10"}`}
                            >
            ...
            {/* VIEW: HOURS MANAGEMENT */}
            {view === "hours" && (
              <div className="max-w-2xl">
                <h1 className="text-4xl font-black mb-10 italic uppercase text-orange-500">Store Hours</h1>
                <form onSubmit={handleUpdateGlobalContent} className="space-y-8 bg-zinc-900/50 p-10 rounded-[40px] border border-zinc-800">
                  <div>
                ...
                <form onSubmit={handleUpdateGlobalContent} className="space-y-8 bg-zinc-900/50 p-10 rounded-[40px] border border-zinc-800">
                  ...
                  <div className="p-6 bg-zinc-800 rounded-3xl space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <button type="button" onClick={() => setSiteContent({ ...siteContent, showModal: !siteContent.showModal })} className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase transition-all ${siteContent.showModal ? "bg-green-600 shadow-[0_0_10px_rgba(22,163,74,0.3)]" : "bg-zinc-600"}`}>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 z-[100]">
            <form onSubmit={handleSaveMenu} className="bg-zinc-900 border border-zinc-800 p-10 rounded-[40px] w-full max-w-lg space-y-5">
              ...
              <div className="space-y-4">
                ...
                {activeCategory === "hoagies" && (
                  <select className="w-full bg-zinc-800 p-4 rounded-xl font-bold text-sm" value={editingItem?.subcategory || ""} onChange={(e) => setEditingItem({ ...editingItem, subcategory: e.target.value })}>
